...
namespace HAGE {
		...
		IndexType DecimateFaces(IndexType nFaces,const UpdateFunction& func = DefaultMergeFunction)
		{
			...
			while(n!=nFaces && decimate(func))
			{
					...
					|| Final()->GetNumFaces() % 10000 == 1)
				{
					...
					printf("\t %i Faces remaining, %i Collapses, ErrMin(%e) ErrMax(%e)\n",(unsigned)Final()->GetNumFaces(),(unsigned)data.size(),fMin,fMax);
				}	
			}
	...
	private:
		...
		Matrix4<_Precision> createQuadricForVert(Vertex v,unsigned* nCount) const
		{
		...
		unsigned computeCollapse(Edge& e)
		{
		...
		void updateEdgeCollapse(const Edge& e,unsigned nCount)
		{
			costItem c;
			c.index = e.Index();
			c.cost = e->Cost;
